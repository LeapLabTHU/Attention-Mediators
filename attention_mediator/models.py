...
class AttentionWithMediators(nn.Module):
    r"""
        AttentionWithMediators
        Implementation of ECCV 2024 paper:
        "Efficient Diffusion Transformer with Step-wise Dynamic Attention Mediators"
        (https://arxiv.org/pdf/2408.05710)

    Args:
        dim (int): Number of input channels.
        latent_size (int): Height/width of the latent space.
        num_heads (int): Number of attention heads.
        qkv_bias (bool): If True, adds a learnable bias to query, key, and value.
        qk_scale (float | None): Overrides the default qk scale of `head_dim ** -0.5` if set.
        attn_drop (float): Dropout rate for attention weights. Default: 0.0.
        proj_drop (float): Dropout rate for the output projection. Default: 0.0.
        shift_size (int): Shift size for attention. Default: 0.
        mediator_num (int): Number of mediators for dynamic attention. Default: 49.
    """

    def __init__(self, dim, latent_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 shift_size=0, mediator_num=64, **kwargs):
    ...
    def forward(self, x, mask=None):
        ...
        mediator_tokens = mediator_tokens.reshape(b, self.mediator_num, num_heads, head_dim).permute(0, 2, 1, 3)
        ...
        mediator_bias1 = mediator_bias1.reshape(1, num_heads, self.mediator_num, -1).permute(0, 1, 3, 2).repeat(b, 1, 1, 1)
...
class SiTBlock(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, latent_size, mediator_num, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        # self.attn = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        self.attn = AttentionWithMediators(
            hidden_size, latent_size=latent_size, num_heads=num_heads,
            qkv_bias=True, attn_drop=0., proj_drop=0.,
            shift_size=0, mediator_num=mediator_num, **block_kwargs
        )
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            SiTBlock(
                hidden_size, 
                num_heads,
                latent_size=(input_size//patch_size, input_size//patch_size),
